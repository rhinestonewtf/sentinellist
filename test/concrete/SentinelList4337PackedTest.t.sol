...
import { SentinelList4337Lib, SENTINEL, ZERO_ADDRESS } from "src/SentinelList4337Packed.sol";
...
contract SentinelList4337Test is Test {
    ...
    function addMany(uint256 amount) public {
        for (uint256 i = 1; i <= amount; i++) {
            ...
            list.push(account, addr, bytes12(0));
        }
    ...
    function test_GetNextWhenEntryIsNotZero() external {
        ...
        list.push(account, address(newEntry), bytes12(hex"4141"));

        address next = list.getNext(account, SENTINEL);
        assertEq(next, newEntry);

        bytes12 data = list.getData(account, newEntry);
        assertEq(data, bytes12(hex"4141"));
    }

    function test_PushRevertWhen_EntryIsZero() external {
        // it should revert
        vm.expectRevert();
        list.push(account, ZERO_ADDRESS, bytes12(0));
    }

    function test_PushRevertWhen_EntryIsSentinel() external {
        // it should revert
        vm.expectRevert();
        list.push(account, SENTINEL, bytes12(0));
    }

    function test_PushRevertWhen_EntryIsAlreadyAdded() external whenEntryIsNotSentinel {
        // it should revert
        address newEntry = makeAddr("newEntry");
        list.push(account, address(newEntry), bytes12(0));
    ...
    function test_SafePushWhenListIsNotInitialized() external {
        ...
        address newEntry = makeAddr("newEntry");
        newList.safePush(account, newEntry, bytes12(0));
    ...
    function test_SafePushRevertWhen_EntryIsZero() external whenListIsInitialized {
        // it should revert
        vm.expectRevert();
        newList.safePush(account, ZERO_ADDRESS, bytes12(0));
    }

    function test_SafePushRevertWhen_EntryIsSentinel() external whenListIsInitialized {
        // it should revert
        vm.expectRevert();
        newList.safePush(account, SENTINEL, bytes12(0));
    }
        ...
        whenEntryIsNotSentinel
    {
        // it should revert
        address newEntry = makeAddr("newEntry");
        newList.safePush(account, address(newEntry), bytes12(0));
