...
library SentinelList4337Lib {
    ...
    struct SentinelList {
        mapping(address key => mapping(address account => Data entry)) entries;
    }
    ...
    function init(SentinelList storage self, address account) internal {
        ...
        self.entries[SENTINEL][account].entry = SENTINEL;
    }
    ...
    function alreadyInitialized(
        ...
        returns (bool)
    {
        return self.entries[SENTINEL][account].entry != ZERO_ADDRESS;
    }
        ...
        returns (address)
    {
        ...
        return self.entries[entry][account].entry;
    }
    ...
    function getData(
        ...
        returns (bytes12)
    {
        return self.entries[entry][account].data;
    }
        ...
        internal
    {
        ...
        if (self.entries[newEntry][account].entry != ZERO_ADDRESS) {
            revert LinkedList_EntryAlreadyInList(newEntry);
        }
        self.entries[newEntry][account].entry = self.entries[SENTINEL][account].entry;
        self.entries[newEntry][account].data = data;
        self.entries[SENTINEL][account].entry = newEntry;
    }
        ...
        internal
    {
        ...
        push({ self: self, account: account, newEntry: newEntry, data: data });
    }
        ...
        internal
    {
        ...
        if (self.entries[prevEntry][account].entry != popEntry) {
            revert LinkedList_InvalidEntry(popEntry);
        }
        self.entries[prevEntry][account].entry = self.entries[popEntry][account].entry;
        delete self.entries[prevEntry][account].data;
        self.entries[popEntry][account].entry = ZERO_ADDRESS;
    }
    ...
    function popAll(SentinelList storage self, address account) internal {
        address next = self.entries[SENTINEL][account].entry;
        while (next != ZERO_ADDRESS) {
            address current = next;
            next = self.entries[next][account].entry;
            self.entries[current][account].entry = ZERO_ADDRESS;
        }
    }
    ...
    function contains(
        ...
        returns (bool)
    {
        return SENTINEL != entry && self.entries[entry][account].entry != ZERO_ADDRESS;
    }
        ...
        returns (address[] memory array, address next)
    {
        ...
        next = self.entries[start][account].entry;
